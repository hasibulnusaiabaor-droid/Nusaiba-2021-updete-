...
import { User, ProfileStats, PinnedContent } from '@/types';
...
export default function UserProfile() {
  ...
  const likedVideos = videos.filter(video => video.likes > 0); // Simplified liked content
  ...
  const stats: ProfileStats = {
    ...
    watchTime: 0, // Would be calculated from actual data
    topVideoViews: Math.max(...userVideos.map(v => v.views), 0),
    monthlyGrowth: 12.5 // Would be calculated from actual data
  };

  // Pinned content (mock data for now)
  const pinnedContent: PinnedContent | null = user.pinnedContentId ? {
